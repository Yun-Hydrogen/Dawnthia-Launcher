...
class Tools(QWidget, Ui_Tools):
    ...
    def TransparentToolButton_1_Clicked(self):
        TeachingTip.create(self.TransparentToolButton_1,
                           ...
                           "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\Dwm\n\
                            下新增OverlayTestMode值为5的DWORD项,可能与多平面覆盖(MPO)\n\
                            有关。如果遇到问题需要卸载，点击“撤销”或者将键值删除即可。",
                           isClosable=True,
                           duration=-1,
    ...
    def PrimaryPushButton_1_Clicked(self):
        ...
        try:
            ...
            mb.yesButton.setText("这是萌萌氢的问题，我要报告问题并惩罚萌萌氢（不要(*。>Д<)o")
            mb.cancelButton.setText("这应该是我的问题ψ(._. )>")
            if mb.exec():
    ...
    def ConfirmButton_Clicked(self):
        ...
        try:
            ...
            mb = MessageBox("操作未完成`(*>﹏<*)′", f"发生了错误：{e.stderr.strip()}", self.window())
            mb.yesButton.setText("这是萌萌氢的问题，我要报告问题并惩罚萌萌氢（(*。>Д<)o")
            mb.cancelButton.setText("这应该是我的问题ψ(._. )>")
            if mb.exec():
                QDesktopServices.openUrl(QUrl("https://github.com/Yun-Hydrogen/Dawnthia-Launcher"))
            else:
